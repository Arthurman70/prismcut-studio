...
class TrackStrip(QWidget):
    ...
    def __init__(self, track, parent=None):
        ...
        for b, attr in ((self.mute, "mute"), (self.solo, "solo")):
            b.setFixedSize(30, 22)
            b.toggled.connect(lambda on, a=attr: (setattr(self.track, a, on),
                                                  self.changed.emit()))
            brow.addWidget(b)
        lay.addLayout(brow)
...
class AudioPanel(QWidget):
    ...
    def __init__(self, project: Project, registry, settings, jobs, get_adapter, parent=None):
        super().__init__(parent)
        ...
        self.wave.setStyleSheet("background:#16181b;border:1px solid #3a3f46;")
    ...
    def _ai_generate(self):
        ...
        if kind == "transcribe":
            ...
            f, _ = QFileDialog.getOpenFileName(self, "Transcribe media", "",
                                               "Media (*.mp3 *.wav *.m4a *.ogg *.flac *.mp4 *.mov *.webm)")
            if not f:
                return
